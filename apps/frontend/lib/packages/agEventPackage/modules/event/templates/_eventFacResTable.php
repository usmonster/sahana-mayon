<table class="expandableTable">
  ...
  <?php foreach($facilityResourceArray as $facRes) : ?>
    <tr>
      ...
      <td class="set125"><a href="<?php echo url_for('event/eventfacilityresource?eventFacilityResourceId='. $facRes['efr_id']); ?>" class="textToForm linkText" name="resourceActivationTime" id="res_act_id_<?php echo $facRes['efr_id']; ?>"><?php echo (isset($facRes['efrat_activation_time']) ? date('m/d/Y H:i', $facRes['efrat_activation_time']) : '----'); ?></a></td>
    </tr>
  <?php endforeach; ?>
  </tbody>
</table>
