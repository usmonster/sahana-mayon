...
class agPluginManager extends sfSymfonyPluginManager
{
  ...
  /**
   *
   * @param boolean integer $status indicating status for which packages you want returned
   * 1 = enabled
   * 0 = disabled
   * @return array of enabled or disabled packages
   */
  static public function getPackagesByStatus($status)
  {
    if ($handle = opendir(sfConfig::get('sf_app_dir') . '/lib/packages/')) {
      $i = 0;
      while (false !== ($file = readdir($handle))) {
        ...
        if ($file != "." && $file != ".." && (strpos($file, 'ag') === 0)) {
          if (agPluginManager::packageStatus($file, sfConfig::get('sf_config_dir')) == $status) {
