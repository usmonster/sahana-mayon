...
//    $sendBack['Username'] = $db->getParameter('username');
//    $sendBack['DSN'] = $db->getParameter('dsn');
//    $sendBack['Password'] = $db->getParameter('password');
    ...
    if (isset($objReader)) {
      ...
      $importHeaders = buildHeaders($objReader, $highestColumnIndex);
      ...
      $importStatus = testHeaders($importFile, $importHeaders, $definitionHeaders);

      if ($importStatus['Success'] == true) {
        $importedStaff = buildImportedStaff($importHeaders, $objReader, $highestRow);
        $returned = buildAndSave($importedStaff);
      } else {
...
//          }
      ...
      foreach ($import as $key => $point) {
        ...
        if (in_array($key, array('Given Name', 'Family Name')) && $point <> null) {
          switch ($key) {
            case "Given Name":
              importName($person,$point,1);
              break;
            case "Family Name":
              importName($person,$point,3);
              break;
          }
        }
// Phone **************************************
        if (in_array($key, array('Home Phone', 'Work Phone', 'Mobile Phone')) && $point <> null) {
          switch ($key) {
            case "Home Phone":
              importPhone($person,$point,2,2);
              break;
            case "Work Phone":
              importPhone($person,$point,1,1);
              break;
            case "Mobile Phone":
              importPhone($person,$point,3,3 );
              break;
          }
        }
// Email **************************************
        if (in_array($key, array('Work Email', 'Personal Email')) && $point <> null) {
          switch ($key) {
            case "Work Email":
              importEmail($person,$point,1,1);
              break;
            case "Personal Email":
              importEmail($person,$point,2,2);
              break;
          }
        }
//Language ************************************
        if ($key == 'Language' && $point <> null) {
          importLanguage($import,$person,$point);
        }
// Address ************************************
        if (in_array($key, array('Home Address', 'Work Address')) && $point <> null) {
          switch ($key) {
            case "Home Address":
              importAddress($person, $point, 1, 1);
              break;
            case "Work Address":
              importAddress($person, $point, 2, 2);
              break;
          }
        }
// Custom *************************************
        if ($key == 'Custom' && $point <> null) {
          importCustom($person, $point);
        }
// Classification *****************************
        ...
        if ($key == 'Classification') {
          importClassification($person, $point, $staff);
        }
...
// importName() *******************
  function importName($person, $point, $type) {
    $nameCheck =
...
// importPhone() ******************
  function importPhone($person, $point, $type, $priority) {
...
// importLanguage() ***************
  function importLanguage($import,$person,$point) {
...
// importClassification() *********
  function importClassification ($person, $point, $staff) {
    if ($point <> null) {
      $resourceTypeCheck =
...
// importAddress() ****************
  function importAddress($person, $point, $type, $priority) {
...
// importCustom() *****************
  function importCustom ($person, $point) {
...
// importEmail() ******************
  function importEmail($person,$point,$type,$priority) {
...
// buildHeaders() *****************
  function buildHeaders($objReader, $highestColumnIndex) {
...
// testHeaders() ******************
  function testHeaders($importFile, $importHeaders, $definitionHeaders) {
    if (array_diff($importHeaders, $definitionHeaders) == null) {
      ...
      $importStatus['Upload Message'] =
          $importFile . ' does not have the right headers to import into Agasti. Please check the column headers of ' . $importFile . ' against the header definitions in your manual to ensure the data is correct.';
