...
abstract class agImportHelper extends agEventHandler
{
  ...
  const             IMPORT_IN_PROGRESS = 'import_in_progress',
                    ...
                    IMPORT_LISTENING = 'import_listening';

  protected static  $importState = array();
  protected         $_conn,
                    $fileInfo,
                    $tempTable,
  ...
  public static function setImportState($importer = NULL, $importStatus = self::IMPORT_LISTENING,
                                        array $importProgress = array(0, 0))
  {
    if (!isset($importer)) {
      return false;
    }
    $importStatus = strtolower($importStatus);
    if (self::IMPORT_LISTENING === $importStatus) {
      unset(self::$importState[$importer]);
    } else {
      self::$importState[$importer]['status'] = $importStatus;
      self::$importState[$importer]['progress'] = $importProgress;
    }
    return self::$importState;
  }
