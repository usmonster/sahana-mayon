...
class agEmbeddedAddressContactsForm extends sfForm
{
  ...
  /**
   * This sets up the agEmbeddedAddressContactForms nedded to create an agPerson.
   * Depending on whether the person is being created or edited, forms are either empty
   * (if creating) or populated (if editing, and the person has, through the entity table an
   * agEntityAddressContact value for the agAddressContactType associated with the embedded form.
   * */
  public function configure()
  {
    ...
    /**
     * The loops below make a 2-d array of line sequence values, inline sequence values, and
     * address element ids. Use it to output fields for address entry.
     * */
    ...
    foreach ($addressElements as $ae) {
      foreach ($ae as $addressElement) {
        ...
        // Hardcoded for now, this sets the input type for state to a dropdown list, populated
    ...
    foreach ($this->address_contact_types as $address_contact_type) {
      ...
      foreach ($this->person->getAgEntity()->getAgEntityAddressContact() as $current) {
        ...
        if ($current->getAddressContactTypeId() == $address_contact_type->getId()) {
          ...
          $type = new agEmbeddedAddressContactForm();
          //$type->setDefault('address_to_type',
          //  $current->getAgAddress()->getAgAddressMjAgAddressValue()->getAgAddressValue());
        }
      }
