...
//      $this['entity_id'],
    ...
    $organizationStaffResources = agOrganization::organizationStaffByResource
            (array($this->getObject()->getId()));
    $personFullName = agPerson::getPersonFullName();

    // Creating an array of staff resource organzation and it's correlating staff name, staff id,
    // and staff resource type for the associated staff resource list box.
    //   array(staff resource organization id => staffInfoString) where staffInfoStringn = staff
    //   primary full name (staff id) : staff resource type.
    ...
    foreach ($organizationStaffResources as $orgStfRes) {
      if ($orgStfRes['staff_resource_organization_id'] == NULL) {
        ...
        $optString = $personFullName[$orgStfRes['person_id']] . ' (' . $orgStfRes['staff_id'] . ')
          : ' . ucwords($staffResourceTypes[$orgStfRes['staff_resource_type_id']]);
    ...
    $availOrgStaffResource = agDoctrineQuery::create()
        ...
        sr.staff_resource_type_id, count(*) as count')
            ...
            ->where('sr.staff_id NOT IN (SELECT ssr.staff_id FROM agStaffResourceOrganization AS ssro
        INNER JOIN ssro.agStaffResource AS ssr WHERE ssro.organization_id=? GROUP BY ssr.staff_id)',
    ...
    // Creating an array of staff resource organzation and it's correlating staff name, staff id,
    // and staff resource type for the available staff resource list box.
    // array(staff resource organization id => staffInfoString) where staffInfoStringn = staff
    // primary full name (staff id) : staff resource type.
    // ie array(1 => Joe Smith (32) : Operator
    $availOrgStaffResourceOption = array();
    foreach ($availOrgStaffResource as $availStfRes) {
      $optString = $personFullName[$availStfRes['agStaff']['person_id']] . ' 
        (' . $availStfRes['staff_id'] . ') : ' . ucwords($staffResourceTypes[$availStfRes
    ...
    $this->setValidators(array(
      ...
      'description' => new sfValidatorString(array('required' => false, 'min_length' => 1,
        'max_length' => 255)),
      'ag_staff_resource_organization_list' => new sfValidatorDoctrineChoice(array('multiple' => true,
        'model' => 'agStaffResourceOrganization', 'required' => false)),
      'ag_staff_resource_non_org_list' => new sfValidatorDoctrineChoice(array('multiple' => true,
        'model' => 'agStaff', 'required' => false)),
    ));

    $this->validatorSchema->setOption('allow_extra_fields', true);
    $this->validatorSchema->setPostValidator(
        new sfValidatorAnd(array(
          new sfValidatorDoctrineUnique(array('model' => 'agStaffResourceOrganization', 'column' =>
            array('id'))),
          new sfValidatorDoctrineUnique(array('model' => 'agStaffResourceOrganization', 'column' =>
            array('staff_resource_id', 'organization_id'))),
        ))
    );
