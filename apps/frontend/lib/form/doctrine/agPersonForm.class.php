...
//  }
  ...
  public function configureWidgets()
  {
     ...
     * */
    $this->setWidget('ag_sex_list', new sfWidgetFormDoctrineChoice(array
          ('multiple' => false, 'model' => 'agSex')));
    $this->setWidget('ag_ethnicity_list', new sfWidgetFormDoctrineChoice(array
          ('multiple' => false, 'model' => 'agEthnicity')));
    $this->setWidget('ag_marital_status_list', new sfWidgetFormDoctrineChoice(array
          ('multiple' => false, 'model' => 'agMaritalStatus')));
  ...
  public function embedDateOfBirthForm()
  {
    ...
    $dateOfBirthForm->getWidgetSchema()->setLabel('date_of_birth', 'Date of Birth');
  ...
  public function embedLanguageForm()
  {
    ...
    //set the max limit for the iterator for the $langSubContainer forms to 3.
    //Then check the languages, if they're greater than 3, reset it to that greater number.
    $m = 3;
    ...
    for ($i = 1; $i <= $m; $i++) {
      ...
      if (isset($personLanguages[$i - 1])) {
        ...
        $languageForm->setDefaults(array(
          ...
          //Right now, priority is hardcoded to the order forms show up in.
      ...
      foreach ($this->ag_person_language_formats as $langFormat) {
        ...
        if ($i <> 1) {
          ...
          $formatForm->widgetSchema->setLabel('language_competency_id',
  ...
  public function saveNameForm($key, $form, $values)
  {
    ...
    if ($form->getObject()->isModified() && $form->getObject()->person_name <> null) {
      ...
      $joinObject = agDoctrineQuery::create()
              ...
              ->where('j.person_name_type_id = ? AND j.person_id = ?',
  ...
  public function saveEmailForm($key, $form, $values)
  {
    ...
    if ($form->getObject()->isModified() && $form->getObject()->email_contact <> null) {
      ...
      $joinObject = agDoctrineQuery::create()
              ...
              ->where('j.email_contact_type_id = ? AND j.entity_id = ?',
  ...
  public function savePhoneForm($key, $form, $values)
  {
    ...
    if ($form->getObject()->isModified() && $form->getObject()->phone_contact <> null) {
      ...
      $joinObject = agDoctrineQuery::create()
              ...
              ->where('j.phone_contact_type_id = ? AND j.entity_id = ?',
                  array($typeId, $this->getObject()->entity_id))
              ->execute()->getFirst();
